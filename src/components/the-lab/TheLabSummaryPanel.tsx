...
const TheLabSummaryPanel = React.memo(function TheLabSummaryPanel({
  ...
  return (
    <div className="w-full lg:col-span-4 space-y-3 sm:space-y-4 lg:sticky lg:top-24 order-1 lg:order-2 self-start">
      ...
      {/* Journey Summary */}
      <div className="liquid-glass relative rounded-xl sm:rounded-2xl p-3 mb-3 sm:mb-4 shadow-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 border border-slate-300/60 dark:border-slate-300/60 dark:border-white/5 overflow-hidden">
        {/* Status Badge - Top Right */}
        <div className="absolute top-0 right-0">
          <div className={cn(
            "px-3 py-1 rounded-bl-xl border-l border-b border-slate-300/60 dark:border-slate-300/60 dark:border-white/5 flex items-center gap-2",
            ...
            !statusOption && "bg-slate-200 dark:bg-slate-100/50 dark:bg-white/5"
          )}>
            <div className={cn(
              ...
              statusOption?.metadata?.color === 'green' && "bg-emerald-600 dark:bg-emerald-400 shadow-emerald-500/20",
              ...
              !statusOption && (
                ...
                selectedStatus === 'confirmed' || selectedStatus === 'booked' ? "bg-emerald-600 dark:bg-emerald-400 shadow-emerald-500/20" :
            ...
            <span className={cn(
              ...
              statusOption?.metadata?.color === 'pink' && "text-slate-600 dark:text-zinc-400",
              statusOption?.metadata?.color === 'blue' && "text-blue-400",
              statusOption?.metadata?.color === 'green' && "text-emerald-600 dark:text-emerald-400",
              statusOption?.metadata?.color === 'amber' && "text-amber-400",
              statusOption?.metadata?.color === 'red' && "text-rose-400",
              !statusOption && (
                selectedStatus === 'draft' ? "text-slate-600 dark:text-zinc-400" :
                selectedStatus === 'sent' ? "text-primary" :
                selectedStatus === 'confirmed' || selectedStatus === 'booked' ? "text-emerald-700 dark:text-emerald-500 dark:text-emerald-400" :
                selectedStatus === 'rejected' ? "text-rose-500 dark:text-rose-400" :
        ...
        <div className="w-full sm:w-auto">
          ...
          <div className="flex flex-wrap gap-3 sm:gap-4 md:gap-8">
            <div className="flex flex-col">
              ...
              <span className="text-slate-900 dark:text-white text-xs font-bold leading-none">{itinerary.itinerary[0]?.areaFocus?.split(',')[0]}</span>
            </div>
            <div className="flex flex-col">
              <span className="text-[9px] font-black text-primary/80 uppercase tracking-widest mb-0.5">Duration</span>
              <span className="text-slate-900 dark:text-white text-xs font-bold leading-none">
      ...
      <div className={cn(
        ...
        isEditingRequirements 
          ? "border-primary/40 bg-white/95 dark:bg-obsidian-dark/95 shadow-[0_0_25px_rgba(255,92,51,0.12)]" 
          : "border-slate-300/60 dark:border-slate-300/60 dark:border-white/5 bg-white/80 dark:bg-obsidian-dark/80"
      )}>
        ...
        <div className="flex items-center justify-between gap-2">
          <button
            ...
            className="flex items-center gap-2 text-slate-900 dark:text-white hover:text-primary transition-colors focus:outline-none min-w-0"
          >
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            {isEditingRequirements ? (
              ...
              <button
                ...
                className="px-2 py-1 text-[10px] font-bold rounded-lg bg-slate-50 dark:bg-slate-100/50 dark:bg-white/5 border border-slate-300/60 dark:border-slate-300/60 dark:border-white/10 hover:bg-primary/20 hover:text-primary-800 dark:hover:text-white hover:border-primary/40 text-primary transition-all flex items-center gap-1 cursor-pointer"
            ...
            <button
              type="button"
              onClick={() => setIsInputsOpen(!isInputsOpen)}
              className="p-1 text-slate-600 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-white transition-colors"
            >
              <ChevronDown 
        ...
        {/* Content Body */}
        {isInputsOpen && (
          <div className="pt-2 border-t border-slate-300/60 dark:border-white/5 space-y-3.5 text-xs animate-in fade-in slide-in-from-top-2 duration-300">
            
            {/* EDIT MODE FORM */}
            {isEditingRequirements ? (
              <div className="space-y-4 max-h-[520px] overflow-y-auto pr-2 border-b border-slate-300/60 dark:border-white/5 pb-2">
                
                {/* Route Locations */}
                <div className="space-y-2 bg-white/[0.02] p-2.5 rounded-xl border border-slate-300/60 dark:border-white/5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <div>
                      <label className="text-[9px] text-slate-600 dark:text-zinc-400 font-bold block mb-1">Starting From *</label>
                      <input
                        type="text"
                        value={editForm.startingLocation}
                        onChange={(e) => setEditForm(prev => ({ ...prev, startingLocation: e.target.value }))}
                        placeholder="e.g. Mumbai"
                        className="w-full bg-slate-50 dark:bg-slate-100/50 dark:bg-white/5 border border-slate-300/60 dark:border-slate-300/60 dark:border-white/10 rounded-lg px-2.5 py-1.5 text-xs text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-zinc-600 focus:outline-none focus:border-primary"
                      />
                    </div>
                    <div>
                      <label className="text-[9px] text-slate-600 dark:text-zinc-400 font-bold block mb-1">Destinations *</label>
                      <input
                        type="text"
                  ...
                  <div>
                    <label className="text-[9px] text-slate-600 dark:text-zinc-400 font-bold block mb-1">Ending Location (Optional)</label>
                    <input
                      type="text"
                ...
                <div className="space-y-2 bg-white/[0.02] p-2.5 rounded-xl border border-slate-300/60 dark:border-white/5">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-[9px] text-slate-600 dark:text-zinc-400 font-bold block mb-1">Start Date *</label>
                      <input
                        type="date"
                        value={editForm.startDate}
                        onChange={(e) => setEditForm(prev => ({ ...prev, startDate: e.target.value }))}
                        className="w-full bg-slate-50 dark:bg-slate-100/50 dark:bg-white/5 border border-slate-300/60 dark:border-slate-300/60 dark:border-white/10 rounded-lg px-2 py-1 text-xs text-white focus:outline-none focus:border-primary"
                      />
                    </div>
                    <div>
                      <label className="text-[9px] text-slate-600 dark:text-zinc-400 font-bold block mb-1">End Date *</label>
                      <input
                        type="date"
                ...
                <div className="space-y-2.5 bg-white/[0.02] p-2.5 rounded-xl border border-slate-300/60 dark:border-white/5">
                  <span className="text-[9px] text-primary font-black uppercase tracking-widest flex items-center gap-1">
                  ...
                  {/* Trip Style */}
                  <div>
                    <label className="text-[9px] text-slate-600 dark:text-zinc-400 font-bold block mb-1.5">Travel Style</label>
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            editForm.tripType === style.id
                              ...
                              : "bg-slate-50 dark:bg-slate-100/50 dark:bg-white/5 border border-slate-300/60 dark:border-slate-300/60 dark:border-white/10 text-slate-600 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-white/10"
                          )}
                        >
                          {style.label}
                        </button>
                  ...
                  {/* Timing Preference */}
                  <div>
                    <label className="text-[9px] text-slate-600 dark:text-zinc-400 font-bold block mb-1.5">Travel Timing</label>
                    <div className="flex flex-wrap gap-1.5">
                      {TIMING_PREFS.map(t => (
                        <button
                          key={t.id}
                  ...
                  <div className="flex items-center gap-3 pt-1">
                    <label className="flex items-center gap-2 cursor-pointer text-xs font-semibold text-slate-800 dark:text-zinc-300">
                      <input
                        type="checkbox"
                        checked={editForm.leisureTime}
                        onChange={(e) => setEditForm(prev => ({ ...prev, leisureTime: e.target.checked }))}
                        className="rounded border-slate-300/60 dark:border-white/10 bg-slate-100/50 dark:bg-white/5 text-primary focus:ring-0"
                      />
                    ...
                    {editForm.leisureTime && (
                      <div className="flex items-center gap-1.5">
                        <span className="text-[10px] text-slate-600 dark:text-zinc-400 font-bold">On Day:</span>
                        <input
                          type="number"
                          min={1}
                          max={30}
                  ...
                  {/* Travel Methods */}
                  <div>
                    <label className="text-[9px] text-slate-600 dark:text-zinc-400 font-bold block mb-1.5">Travel Methods</label>
                    <div className="flex flex-wrap gap-1.5">
                ...
                <div className="space-y-1">
                  <label className="text-[9px] text-slate-600 dark:text-zinc-400 font-bold block">Custom Daywise Plan (Optional)</label>
                  <textarea
                    rows={2}
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-100/50 dark:bg-white/5 border border-slate-300/60 dark:border-slate-300/60 dark:border-white/10 rounded-lg p-2 text-xs text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-zinc-600 focus:outline-none focus:border-primary resize-none"
                  />
                </div>
                ...
                <div className="space-y-2">
                  <div>
                    <label className="text-[9px] text-emerald-600 dark:text-emerald-400 font-bold block mb-1 uppercase tracking-wider">Must Include Notes</label>
                    <textarea
                      rows={2}
                      ...
                      className="w-full bg-emerald-500/5 border border-emerald-500/20 rounded-lg p-2 text-xs text-slate-900 dark:text-zinc-200 placeholder:text-slate-400 dark:placeholder:text-zinc-600 focus:outline-none focus:border-emerald-500/50 resize-none"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-rose-500/5 border border-rose-500/20 rounded-lg p-2 text-xs text-slate-900 dark:text-zinc-200 placeholder:text-slate-400 dark:placeholder:text-zinc-600 focus:outline-none focus:border-rose-500/50 resize-none"
                    />
                  </div>
                </div>
                ...
                <div className="p-2.5 rounded-xl bg-gradient-to-r from-purple-900/20 to-indigo-900/20 border border-purple-500/30 space-y-1.5">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={cn(
                      ...
                      isFeedbackPromptOverLimit ? "text-red-400" : "text-slate-600 dark:text-zinc-400"
                    )}>
                  ...
                  <textarea
                    ...
                    className={cn(
                      "w-full bg-white/80 dark:bg-black/40 border rounded-lg p-2 text-xs text-slate-900 dark:text-white placeholder:text-slate-500 dark:placeholder:text-purple-300/40 focus:outline-none resize-none transition-colors",
                ...
                <div className="pt-2 border-t border-slate-300/60 dark:border-white/10 flex flex-col sm:flex-row items-center justify-between gap-2">
                  <button
                    type="button"
              ...
              <>
                  ...
                  <div className="flex gap-2">
                    ...
                    <div className="space-y-2 w-full">
                      <div>
                        ...
                        <span className="text-slate-900 dark:text-zinc-200 font-semibold">{tripMetadata.startingLocation || "—"}</span>
                      </div>
                      <div>
                        <span className="text-[9px] text-zinc-500 font-bold uppercase tracking-widest block">Destinations</span>
                        <span className="text-slate-900 dark:text-zinc-200 font-semibold">{tripMetadata.destinations || "—"}</span>
                      </div>
                      {tripMetadata.endingLocation && (
                        <div>
                          ...
                          <span className="text-slate-900 dark:text-zinc-200 font-semibold">{tripMetadata.endingLocation}</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 w-full">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {tripMetadata.tripType && (
                        <div className="flex items-center gap-2 px-2.5 py-1 rounded-md bg-slate-50 dark:bg-slate-100/50 dark:bg-white/5 border border-slate-300/60 dark:border-slate-300/60 dark:border-white/10 hover:bg-white/10 transition-colors">
                          <span className="text-primary/70 font-black uppercase tracking-widest text-[8px]">Style</span>
                          <span className="text-slate-900 dark:text-white font-bold capitalize text-[10px]">{tripMetadata.tripType}</span>
                        </div>
                      )}
                      {tripMetadata.travelTimePreference && tripMetadata.travelTimePreference !== "no_preference" && (
                        <div className="flex items-center gap-2 px-2.5 py-1 rounded-md bg-slate-50 dark:bg-slate-100/50 dark:bg-white/5 border border-slate-300/60 dark:border-slate-300/60 dark:border-white/10 hover:bg-white/10 transition-colors">
                          ...
                          <span className="text-slate-900 dark:text-white font-bold capitalize text-[10px]">{tripMetadata.travelTimePreference.replace(/_/g, ' ')}</span>
                        </div>
                      )}
                      {tripMetadata.leisureTime && (
                        <div className="flex items-center gap-2 px-2.5 py-1 rounded-md bg-slate-50 dark:bg-slate-100/50 dark:bg-white/5 border border-slate-300/60 dark:border-slate-300/60 dark:border-white/10 hover:bg-white/10 transition-colors">
                          <span className="text-primary/70 font-black uppercase tracking-widest text-[8px]">Leisure</span>
                          <span className="text-slate-900 dark:text-white font-bold text-[10px]">Day {tripMetadata.leisureDay || "N/A"}</span>
                        </div>
                      )}
                    </div>
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {tripMetadata.travelMethods.map((method: string, i: number) => (
                            <span key={i} className="px-2 py-0.5 rounded bg-slate-50 dark:bg-slate-100/50 dark:bg-white/5 border border-slate-300/60 dark:border-slate-300/60 dark:border-white/10 text-[9px] font-semibold text-slate-800 dark:text-zinc-300">
                              {method}
                            </span>
                          ))}
                        </div>
                ...
                {tripMetadata.daywiseDestinations && (
                  <div className="flex gap-2 border-t border-white/[0.03] pt-2">
                    ...
                    <div className="space-y-0.5 w-full">
                      ...
                      <p className="text-slate-800 dark:text-zinc-300 text-[10px] leading-relaxed bg-slate-100/50 dark:bg-white/5 border border-slate-300/60 dark:border-white/5 rounded-lg p-2 mt-1">
                        {tripMetadata.daywiseDestinations}
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="space-y-2 w-full">
                      ...
                      {tripMetadata.mustInclude && (
                        <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-lg px-2.5 py-1.5">
                          <span className="text-emerald-600 dark:text-emerald-400 font-bold block mb-0.5 uppercase tracking-widest text-[8px]">Must Include</span>
                          <span className="text-slate-800 dark:text-zinc-300 leading-normal text-[10px]">{tripMetadata.mustInclude}</span>
                        </div>
                      )}
                      {tripMetadata.avoid && (
                        <div className="bg-rose-500/5 border border-rose-500/10 rounded-lg px-2.5 py-1.5">
                          ...
                          <span className="text-slate-800 dark:text-zinc-300 leading-normal text-[10px]">{tripMetadata.avoid}</span>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {onRegenerateItinerary && (
                  <div className="pt-3 border-t border-slate-300/60 dark:border-white/10 space-y-2">
                    <div className="space-y-1">
                      <div className="relative flex items-center">
                        <input
                          ...
                          className={cn(
                            "w-full bg-slate-100/50 dark:bg-white/5 border rounded-xl px-3 py-2 pr-9 text-xs text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-zinc-500 focus:outline-none transition-colors",
                            isQuickPromptOverLimit ? "border-red-500/60 focus:border-red-500" : "border-slate-300/60 dark:border-white/10 focus:border-purple-500/50"
                          )}
                        />
                      ...
                      {quickPrompt.trim().length > 0 && (
                        <div className="flex justify-between items-center text-[10px] px-1">
                          <span className={cn(
                            "font-semibold",
                            isQuickPromptOverLimit ? "text-red-400" : "text-slate-600 dark:text-zinc-400"
                          )}>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {onOpenAddClient && (
            <Button
              ...
              className="h-6 text-[10px] text-indigo-400 hover:text-indigo-300 hover:bg-slate-100 dark:hover:bg-slate-100/50 dark:bg-white/5 px-2 rounded-md font-semibold gap-1"
            >
          ...
          if (!selectedClient) {
            return (
              <div className="flex flex-col items-center justify-center py-4 text-center">
                <div className="w-10 h-10 rounded-full bg-slate-50 dark:bg-slate-100/50 dark:bg-white/5 border border-slate-300/60 dark:border-slate-300/60 dark:border-white/10 flex items-center justify-center mb-3">
                ...
                {onOpenAddClient && (
                  <Button
                    ...
                    size="sm"
                    onClick={onOpenAddClient}
                    className="h-7 text-[10px] bg-slate-100/50 dark:bg-white/5 border-slate-300/60 dark:border-white/10 text-zinc-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-white/10 rounded-lg gap-1.5 font-medium"
                  >
            ...
            <>
              <div className="flex items-center gap-4 mb-4">
                ...
                <div className="min-w-0">
                  <p className="font-extrabold text-slate-900 dark:text-white text-base truncate">{selectedClient.name}</p>
              ...
              <div className="grid grid-cols-2 gap-2 mb-3">
                {selectedClient.email && (
                  <div className="p-3 rounded-xl border border-slate-300/60 dark:border-white/5 bg-slate-100/50 dark:bg-white/5 flex flex-col justify-center col-span-2">
                    <p className="text-[8px] text-slate-500 font-black uppercase tracking-widest mb-0.5">Email</p>
                    <p className="text-[10px] font-bold text-slate-900 dark:text-slate-300 truncate">{selectedClient.email}</p>
                  </div>
                )}
                {selectedClient.phone && (
                  <div className="p-3 rounded-xl border border-slate-300/60 dark:border-white/5 bg-slate-100/50 dark:bg-white/5 flex flex-col justify-center col-span-2">
                    <p className="text-[8px] text-slate-500 font-black uppercase tracking-widest mb-0.5">Phone</p>
                    <p className="text-[10px] font-bold text-slate-900 dark:text-slate-300">{selectedClient.phone}</p>
                  </div>
                )}
                <div className="p-3 rounded-xl border border-slate-300/60 dark:border-white/5 bg-slate-100/50 dark:bg-white/5 flex flex-col justify-center">
              ...
              {selectedClient.notes && (
                <div className="p-3 rounded-xl border border-slate-300/60 dark:border-white/5 bg-slate-100/50 dark:bg-white/5">
                  <p className="text-[8px] text-slate-500 font-black uppercase tracking-widest mb-1">Notes</p>
                  <p className="text-[10px] leading-relaxed text-slate-600 dark:text-slate-400 line-clamp-3">{selectedClient.notes}</p>
                </div>
              )}
              {tagList.length > 0 && (
