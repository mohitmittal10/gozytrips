            ...
            <div data-pdf-section="cover" style={{ paddingBottom: "10px" }}>
                <div style={{ height: "240px", overflow: "hidden", position: "relative", background: "#0f172a" }}>
                    ...
                    {(agent.logoUrl || agent.companyName) && (
                        <div style={{ position: "absolute", top: "22px", right: "30px", zIndex: 10, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(255,255,255,0.92)", backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,0.4)", borderRadius: "8px", padding: "8px 14px", boxShadow: "0 4px 16px rgba(0,0,0,0.12)" }}>
                            {agent.logoUrl ? (
                                <img src={agent.logoUrl} alt={agent.companyName} crossOrigin="anonymous" style={{ maxHeight: "28px", maxWidth: "80px", objectFit: "contain", display: "block" }} />
                            ) : (
                                <div style={{ width: "26px", height: "26px", borderRadius: "6px", background: "rgba(15,23,42,0.8)", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontWeight: 800, fontSize: "12px" }}>
                ...
                <div style={{ padding: "45px", background: "rgba(255,255,255,0.42)" }}>
                    <p style={{ fontSize: "14px", color: "#64748b", margin: "0 0 35px 0", lineHeight: "1.7", fontWeight: 500 }}>
                        {agent.tagline || (agent.agentName ? `${itinerary.itinerary?.length || 0}-day bespoke journey · Curated by ${agent.agentName}` : `${itinerary.itinerary?.length || 0}-day bespoke journey`)}
                    </p>

                    {/* Stat cards row */}
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "16px", marginBottom: "40px", pageBreakInside: "avoid" }}>
                        {[
                            clientName ? { label: "Guest / Client", value: clientName } : null,
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: clientName && (agent.companyName || agent.agentName) ? "1fr 1fr" : "1fr", gap: "24px", paddingTop: "30px", borderTop: "1px solid #e2e8f0", pageBreakInside: "avoid" }}>
                        {/* Client Details Box */}
                        {clientName ? (
                        ...
                        {(agent.companyName || agent.agentName || agent.agentPhone || agent.agentEmail || agent.agentWebsite) ? (
                            <div style={{ padding: "24px", background: "rgba(255,255,255,0.7)", border: "1px solid rgba(148,163,184,0.24)", borderTop: "3px solid #3b82f6", borderRadius: "8px", boxShadow: "0 8px 24px rgba(15,23,42,0.04)" }}>
                                ...
                                {agent.companyName && <p style={{ fontWeight: 800, color: "#0f172a", fontSize: "16px", margin: "0 0 4px 0", fontFamily: "'Outfit', sans-serif" }}>{agent.companyName}</p>}
                                {agent.agentName && <p style={{ fontSize: "12px", color: "#64748b", margin: "0 0 10px 0", fontWeight: 600 }}>Consultant: <strong style={{ color: "#0f172a" }}>{agent.agentName}</strong></p>}
                        ...
                        {cabs.map((c, i) => (
                            <div key={`cab-${i}`} style={{ display: "flex", gap: "20px", border: "1px solid rgba(148,163,184,0.2)", padding: "16px", background: "white" }}>
                                ...
                                <div style={{ flex: 1 }}>
                                    ...
                                    {c.vehicleType && <h4 style={{ margin: "0 0 10px 0", fontSize: "16px", color: "#0f172a", fontWeight: 700 }}>{c.vehicleType}</h4>}
                                    <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "4px", fontSize: "11px", color: "#475569" }}>
                                        {c.route && <div style={{ display: "flex", justifyContent: "space-between" }}><span>RTE</span><span style={{ fontWeight: 600 }}>{c.route}</span></div>}
                                        {c.pickupTime && <div style={{ display: "flex", justifyContent: "space-between" }}><span>PICK</span><span style={{ fontWeight: 600 }}>{c.pickupTime}</span></div>}
                        ...
                        {buses.map((b, i) => (
                            <div key={`bus-${i}`} style={{ display: "flex", gap: "20px", border: "1px solid rgba(148,163,184,0.2)", padding: "16px", background: "white" }}>
                                ...
                                <div style={{ flex: 1 }}>
                                    ...
                                    {b.busType && <h4 style={{ margin: "0 0 10px 0", fontSize: "16px", color: "#0f172a", fontWeight: 700 }}>{b.busType}</h4>}
                                    <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "4px", fontSize: "11px", color: "#475569" }}>
                                        {b.route && <div style={{ display: "flex", justifyContent: "space-between" }}><span>RTE</span><span style={{ fontWeight: 600 }}>{b.route}</span></div>}
                                        {b.departureTime && <div style={{ display: "flex", justifyContent: "space-between" }}><span>DEP</span><span style={{ fontWeight: 600 }}>{b.departureTime}</span></div>}
