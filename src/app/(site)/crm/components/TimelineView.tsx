    ...
    if (!hasTrips) {
        return (
            <div className="mt-4 bg-white/[0.03] backdrop-blur-xl border border-white/[0.08] rounded-2xl p-16 text-center text-zinc-300 flex flex-col items-center justify-center">
                <Calendar className="w-12 h-12 text-zinc-400 mb-4 opacity-60" />
                <h3 className="text-xl font-bold text-white mb-2">No Trips Yet</h3>
                <p className="text-zinc-400 text-sm">Create an itinerary in The Lab to see the timeline here.</p>
            </div>
        );
    }
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 h-[50vh] overflow-y-auto pr-1">
                        {enrichedClients.filter(c => c.allTrips.length > 0).map(client => (
                            <div key={client.id} className="space-y-2">
                                <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider px-1">{client.name}</p>
                                {client.allTrips.map((trip: any) => (
                                    <button
                                        key={trip.id}
                                        onClick={() => setSelectedTripForModal(trip)}
                                        className="w-full flex flex-col p-3 rounded-xl bg-white/5 border border-white/5 hover:border-primary/30 hover:bg-white/10 text-left transition-all group"
                                    >
                                        <p className="text-xs font-semibold text-white truncate group-hover:text-primary">
                    ...
                    <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-3">
                            <Button
                                variant="ghost"
                                size="sm"
                                className="h-8 text-zinc-300 hover:text-white hover:bg-white/10 rounded-xl"
                                onClick={() => setSelectedTripForModal(null)}
                            >
                                <ChevronLeft className="w-4 h-4 mr-1" /> Back to Trips
                            </Button>
                            <p className="text-sm text-zinc-400">
                                Viewing: <span className="text-white font-semibold">{selectedTripForModal?.title || selectedTripForModal?.destinations || 'Trip Timeline'}</span>
                            </p>
                        </div>
                        <div className="flex items-center gap-3">
                            <Select 
                            ...
                            >
                                <SelectTrigger className="w-[150px] bg-white/5 border-white/10 text-white h-9 rounded-xl">
