    ...
    if (!loading && !error && itineraries.length === 0) {
        return (
            <div className="mt-4 bg-white/60 dark:bg-white/[0.03] backdrop-blur-xl border border-slate-200 dark:border-white/[0.08] rounded-2xl p-16 text-center text-slate-700 dark:text-zinc-300 flex flex-col items-center justify-center gap-3">
                ...
                <Link href="/the-lab">
                    <Button className="mt-2 px-6 py-2.5 aurora-gradient text-slate-900 dark:text-white rounded-xl text-sm font-bold hover:brightness-110 transition-all flex items-center gap-2 h-10 border-none">
        ...
        <div className="mt-4 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                ...
                <div className="relative flex-1 min-w-0">
                    <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-600 dark:text-zinc-400 pointer-events-none" />
                    <Input
                        placeholder="Search by title, destination, client…"
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        className="pl-10 h-10 bg-slate-200 dark:bg-black/40 border-slate-300 dark:border-white/10 text-slate-900 dark:text-white placeholder:text-slate-600 dark:text-zinc-400 focus-visible:ring-primary/40 rounded-xl text-sm"
                    />
                </div>

                {/* Status filter */}
                <Select value={statusFilter} onValueChange={setStatusFilter}>
                    <SelectTrigger className="w-full sm:w-[160px] h-10 bg-slate-100 dark:bg-white/5 border-slate-300 dark:border-white/10 text-slate-900 dark:text-white rounded-xl text-sm">
                        <Filter className="w-3.5 h-3.5 mr-2 text-slate-600 dark:text-zinc-400" />
                ...
                <Link href="/the-lab">
                    <Button className="h-10 px-4 aurora-gradient text-slate-900 dark:text-white rounded-xl text-sm font-bold hover:brightness-110 transition-all flex items-center gap-2 border-none whitespace-nowrap">
            ...
            <div className="bg-white/60 dark:bg-white/[0.03] backdrop-blur-xl border border-slate-200 dark:border-white/[0.08] rounded-2xl overflow-hidden shadow-xl">
                {loading ? (
                    ...
                    <div className="divide-y divide-white/5">
                        {Array.from({ length: 6 }).map((_, i) => (
                            <div key={i} className="flex items-center gap-3 p-4">
                                <div className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-white/5 animate-pulse shrink-0" />
                                <div className="flex-1 space-y-1.5">
                                    <div className="h-3.5 w-48 bg-slate-200 dark:bg-white/10 rounded animate-pulse" />
                    ...
                    <div className="flex flex-col items-center justify-center py-16 gap-3 text-slate-600 dark:text-zinc-400">
                        <Search className="w-8 h-8 opacity-40 text-slate-600 dark:text-zinc-400" />
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-0.5">
                                            {destination && (
                                                <span className="text-[11px] text-slate-600 dark:text-zinc-400 flex items-center gap-1 font-medium">
                                            ...
                                            {trip.client_name && (
                                                <span className="text-[11px] text-slate-600 dark:text-zinc-400 flex items-center gap-1">
                                                    <Users className="w-3 h-3 shrink-0 text-slate-600 dark:text-zinc-400" />
                                                    {trip.client_name}
                                                </span>
                                            )}
                                            {trip.start_date && (
                                            ...
                                            {totalPax > 0 && (
                                                <span className="text-[11px] text-slate-600 dark:text-zinc-400">{totalPax} pax</span>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    {/* Updated at */}
                                    <span className="hidden lg:block text-[10px] text-slate-600 dark:text-zinc-400 font-medium shrink-0 w-[72px] text-right">
                                    ...
                                    <div className="flex items-center gap-1 opacity-70 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <Button
                                            variant="ghost"
                                            size="icon"
                                            className="h-8 w-8 text-slate-600 dark:text-zinc-400 hover:text-red-400 hover:bg-red-400/10 rounded-lg"
            ...
            {!loading && filtered.length > 0 && totalPages > 1 && (
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 rounded-2xl border border-white/[0.08] bg-white/[0.03] backdrop-blur-xl mt-4">
                    <p className="text-xs text-slate-600 dark:text-zinc-400 font-medium">
                        Page <span className="text-slate-900 dark:text-white font-bold">{currentPage}</span> of{" "}
                        <span className="text-slate-900 dark:text-white font-bold">{totalPages}</span> ({filtered.length} total)
                    </p>
                    <div className="flex items-center gap-2">
                        <Button
                            ...
                            className="h-8 w-8 rounded-xl bg-slate-100 dark:bg-white/5 border-slate-300 dark:border-white/10 text-slate-600 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-white/10 disabled:opacity-30 transition-all cursor-pointer disabled:cursor-not-allowed"
                            title="Previous page"
                        >
                                ...
                                return (
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            currentPage === pageNum
                                                ? "bg-primary/20 border border-primary/40 text-slate-900 dark:text-white font-bold"
