              ...
              <div className={cn(
                ...
                <p className={cn("text-[10px] mt-1 opacity-80", isAgent ? "text-primary-foreground/90 text-right" : "text-zinc-400")}>
                  ...
                  {isAgent ? (
                    ...
                    !msg.is_read && (
                      <span className="ml-1.5 text-amber-300 font-semibold">● unread</span>
                    )
                  )}
                </p>
              </div>
...
function AgentControlsPanel({ response, formId, onUpdated }: { response: ResponseDetailSheetProps["response"]; formId: string; onUpdated?: () => void }) {
  ...
  return (
    <div className="space-y-5 px-5 py-4">
      ...
      {saveError && (
        <div className="flex items-center gap-2 p-2.5 bg-rose-500/10 border border-rose-500/20 rounded-xl text-xs text-rose-300">
