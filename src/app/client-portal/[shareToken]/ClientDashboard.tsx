...
function TripSnapshot({ response }: { response: EnquiryResponseData }) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      {items.map((item, i) =>
        item.value ? (
          <div key={i} className="flex items-start gap-3 bg-slate-100/70 dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.06] rounded-xl p-3">
            <div className="p-1.5 bg-[#FF5C33]/10 rounded-lg text-[#FF5C33] shrink-0 mt-0.5">
              {item.icon}
            </div>
            <div className="min-w-0">
              <p className="text-[10px] text-slate-500 dark:text-gray-600 uppercase tracking-wider font-semibold">{item.label}</p>
              <p className="text-sm text-slate-900 dark:text-zinc-200 mt-0.5 font-medium break-words">{item.value}</p>
            </div>
          </div>
        ) : null
      )}
      {response.travel_methods && response.travel_methods.length > 0 && (
        <div className="sm:col-span-2 flex items-center gap-3 bg-slate-100/70 dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.06] rounded-xl p-3">
          <div className="p-1.5 bg-[#FF5C33]/10 rounded-lg text-[#FF5C33] shrink-0">
            <Plane className="w-4 h-4" />
          </div>
          <div>
            <p className="text-[10px] text-slate-500 dark:text-gray-600 uppercase tracking-wider font-semibold mb-1.5">Travel Methods</p>
            <div className="flex flex-wrap gap-1.5">
              {response.travel_methods.map((m) => (
                <span key={m} className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-200 dark:bg-white/5 border border-slate-300 dark:border-white/10 rounded-full text-xs text-slate-800 dark:text-gray-300 font-medium">
                  {METHOD_ICONS[m]} {m}
                </span>
              ))}
            </div>
...
function ChatWidget({ responseId, agentName }: { responseId: string; agentName: string }) {
  ...
  return (
    <div className="flex flex-col h-full" style={{ minHeight: 400 }}>
          ...
          return (
            <div key={msg.id} className={cn("flex", isMe ? "justify-end" : "justify-start")}>
              ...
              <div className={cn(
                ...
                isMe
                  ? "bg-gradient-to-br from-[#FF5C33] to-[#EC4899] text-white rounded-br-sm font-medium"
                  : "bg-slate-100 dark:bg-white/[0.07] border border-slate-200 dark:border-white/[0.08] text-slate-900 dark:text-zinc-200 rounded-bl-sm font-medium"
              )}>
                <p className="text-sm leading-relaxed break-words whitespace-pre-wrap">{msg.body}</p>
                <p className={cn("text-[10px] mt-1 opacity-60", isMe ? "text-white/80 text-right" : "text-slate-500 dark:text-gray-500")}>
      ...
      {/* Input bar */}
      <div className="shrink-0 border-t border-slate-200 dark:border-white/[0.06] p-3">
        <div className="flex items-end gap-2 bg-slate-100 dark:bg-white/[0.04] border border-slate-300 dark:border-white/[0.08] rounded-2xl p-1 pl-3 focus-within:border-[#FF5C33]/30 transition-colors">
          <textarea
            ...
            rows={1}
            className="flex-1 bg-transparent text-sm text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-gray-600 focus:outline-none resize-none py-2 leading-relaxed max-h-28 overflow-y-auto"
            style={{ minHeight: 36 }}
          />
          <button
            onClick={handleSend}
            ...
            className="shrink-0 w-9 h-9 rounded-xl bg-gradient-to-br from-[#FF5C33] to-[#EC4899] text-white flex items-center justify-center hover:brightness-110 transition-all disabled:opacity-40 disabled:cursor-not-allowed shadow-lg shadow-[#FF5C33]/20 cursor-pointer"
          >
            {sending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}
          </button>
        </div>
        <p className="text-[10px] text-slate-500 dark:text-gray-700 mt-1.5 ml-1 font-medium">Shift+Enter for new line · Enter to send</p>
      </div>
    </div>
  );
}
...
function ItinerarySubTabs({
  ...
  return (
    <div className="space-y-4">
      {/* Sub-tab bar */}
      <div className="flex gap-1 bg-slate-100 dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.07] rounded-2xl p-1 overflow-x-auto no-scrollbar">
        {tabDefs.map((t) => (
          <button
            key={t.key}
            onClick={() => setTab(t.key)}
            className={cn(
              "flex-1 min-w-fit px-3 py-2 rounded-xl text-xs font-semibold transition-all whitespace-nowrap cursor-pointer",
              tab === t.key
                ? "bg-white dark:bg-zinc-800 border border-slate-200 dark:border-white/10 text-slate-900 dark:text-white shadow-sm"
                : "text-slate-600 dark:text-gray-500 hover:text-slate-900 dark:hover:text-gray-300"
            )}
          >
            {t.label}
          </button>
...
export function ClientDashboard({ formMeta, shareToken, responseId }: ClientDashboardProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-[#05070A] text-slate-900 dark:text-white transition-colors">
      {/* ── Sticky Header ── */}
      <header className="sticky top-0 z-20 bg-white/90 dark:bg-[#05070A]/90 backdrop-blur-xl border-b border-slate-200 dark:border-white/[0.06]">
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            {formMeta.agent_avatar_url ? (
              <img src={formMeta.agent_avatar_url} alt={agentName} className="w-8 h-8 rounded-full border border-slate-200 dark:border-white/10 object-cover shrink-0" />
            ) : (
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-slate-600 dark:text-gray-400 font-medium truncate">{formMeta.title}</p>
            </div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              title="Sign out"
              className="p-2 text-slate-500 dark:text-gray-600 hover:text-red-500 transition-colors rounded-lg hover:bg-red-500/10 cursor-pointer"
            >
        ...
        <div className="max-w-3xl mx-auto px-4 pb-0 flex gap-1">
          ...
          <button
            ...
            className={cn(
              "relative px-4 py-2.5 text-xs font-semibold capitalize transition-all border-b-2 cursor-pointer",
              activeTab === "overview"
                ? "text-slate-900 dark:text-white border-[#FF5C33]"
                : "text-slate-600 dark:text-gray-500 border-transparent hover:text-slate-900 dark:hover:text-gray-300"
            )}
          >
            Overview
          </button>
          ...
          <button
            ...
            className={cn(
              "relative px-4 py-2.5 text-xs font-semibold capitalize transition-all border-b-2 flex items-center gap-1.5 cursor-pointer",
              activeTab === "itinerary"
                ? "text-slate-900 dark:text-white border-[#FF5C33]"
                : "text-slate-600 dark:text-gray-500 border-transparent hover:text-slate-900 dark:hover:text-gray-300"
            )}
          ...
          <button
            ...
            className={cn(
              "relative px-4 py-2.5 text-xs font-semibold capitalize transition-all border-b-2 flex items-center gap-1 cursor-pointer",
              activeTab === "messages"
                ? "text-slate-900 dark:text-white border-[#FF5C33]"
                : "text-slate-600 dark:text-gray-500 border-transparent hover:text-slate-900 dark:hover:text-gray-300"
            )}
      ...
      <main className="flex-1 max-w-3xl mx-auto w-full px-4 py-6">
        ...
        {activeTab === "overview" && (
          <div className="space-y-5">
            ...
            {/* Hero greeting */}
            <div className="relative bg-white/80 dark:bg-gradient-to-br dark:from-[#FF5C33]/8 dark:via-[#EC4899]/5 dark:to-[#7C3AED]/8 border border-slate-200 dark:border-white/[0.06] rounded-2xl p-6 overflow-hidden shadow-sm">
              ...
              <div className="relative z-10">
                ...
                <p className="text-slate-600 dark:text-gray-400 text-sm mt-2 max-w-lg font-medium">
                  Track your enquiry status, view your trip details, and chat directly with {agentName}.
                </p>
                <div className="mt-4 flex items-center gap-2 text-xs text-slate-500 dark:text-gray-500 font-medium">
                  <Clock className="w-3.5 h-3.5 text-[#FF5C33]" />
            ...
            {response.agent_note && (
              <div className="bg-[#FF5C33]/5 border border-[#FF5C33]/15 rounded-2xl p-4 flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-800 dark:text-zinc-200 leading-relaxed font-medium">{response.agent_note}</p>
                  {response.agent_note_updated_at && (
                    <p className="text-[10px] text-slate-500 dark:text-gray-600 mt-1.5">
            ...
            {response.itinerary_share_url && (
              <a
                ...
                className="group flex items-center justify-between p-5 bg-purple-500/5 dark:bg-gradient-to-r dark:from-[#7C3AED]/10 dark:to-[#EC4899]/10 border border-purple-500/20 rounded-2xl hover:border-purple-500/40 transition-all shadow-sm"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-slate-900 dark:text-white">View Your Itinerary</p>
                    <p className="text-xs text-slate-500 dark:text-gray-500 mt-0.5">Your personalised travel plan is ready</p>
                  </div>
                </div>
            ...
            {/* Status Timeline */}
            <div className="bg-white/80 dark:bg-white/[0.02] border border-slate-200 dark:border-white/[0.06] rounded-2xl p-5 shadow-sm">
              <p className="text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-gray-500 mb-5 flex items-center gap-2">
            ...
            {/* Trip Snapshot */}
            <div className="bg-white/80 dark:bg-white/[0.02] border border-slate-200 dark:border-white/[0.06] rounded-2xl p-5 shadow-sm">
              <p className="text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-gray-500 mb-4 flex items-center gap-2">
              ...
              {response.special_requests && (
                <div className="mt-4 pt-4 border-t border-slate-200 dark:border-white/5">
                  <p className="text-[10px] text-slate-500 dark:text-gray-600 uppercase tracking-wider font-semibold mb-1.5">Special Requests</p>
                  <p className="text-sm text-slate-700 dark:text-gray-300 leading-relaxed font-medium">{response.special_requests}</p>
                </div>
              )}
            </div>
            ...
            <button
              ...
              className="w-full flex items-center justify-between p-4 bg-white/80 dark:bg-white/[0.02] border border-slate-200 dark:border-white/[0.06] rounded-2xl hover:bg-slate-100 dark:hover:bg-white/[0.04] hover:border-[#FF5C33]/20 transition-all group shadow-sm cursor-pointer"
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <p className="text-sm font-semibold text-slate-900 dark:text-white">Chat with {agentName}</p>
                  <p className="text-xs text-slate-500 dark:text-gray-500">Ask questions or share additional details</p>
                </div>
              </div>
          ...
          <div className="space-y-5 pb-8">
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-white/5 cursor-pointer"
        ...
        {/* ══ MESSAGES TAB ══ */}
        {activeTab === "messages" && (
          <div className="bg-white/80 dark:bg-white/[0.02] border border-slate-200 dark:border-white/[0.06] rounded-2xl overflow-hidden shadow-sm" style={{ height: "calc(100vh - 160px)" }}>
            <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-200 dark:border-white/[0.06]">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-500 dark:text-gray-500 flex items-center gap-1 font-medium">
